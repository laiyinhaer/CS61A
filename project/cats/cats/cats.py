...
def about(topic):
    ...
    def slec(lst):
        res = []
        modified_lst = split(lower(remove_punctuation(lst)))
        for exam in topic:
            for comp in modified_lst:
                if exam == comp:
                    return True
        return False
    return slec
...
def accuracy(typed, reference):
    ...
    if typed_words == [] and reference_words == []:
        ...
        while len(reference_words) != 0:
            if len(typed_words) == 0:
                return total / total_length * 100
            if reference_words.pop(0) == typed_words.pop(0):
                total += 1
        return 100 * (total / total_length)     
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    min_word, min_diff = word_list[0], diff_function(typed_word, word_list[0], limit)
...
def feline_fixes(typed, reference, limit):
    ...
    if len(typed) == 0 or len(reference) == 0:
        return abs(len(typed)- len(reference))
...
def hidden_kittens(typed, reference, limit):
    ...
    def rep(typed, reference, limit):
        ...
        if len(reference) == 0:
            ...
            return 1
        if typed[0] == reference[0]:
            return rep(typed[1:], reference[1:], limit) + rep(typed[1:], reference, limit)
        if typed[0] != reference[0]:
            return rep(typed[1:], reference, limit)
    ans =  rep(typed, reference, limit)
...
def fastest_words(match):
    ...
    for index in word_indices:
        each_word_fast = time(match, 0, index)
        each_word_player = 0
        for i in player_indices:
            word_time = time(match, i, index)
            if each_word_fast > word_time:
                each_word_fast = word_time
                each_word_player = i
        fastest[each_word_player].append(get_word(match, index))
    return fastest
