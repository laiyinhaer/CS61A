...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    index, this, total, flag = 0, 0, 0, False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            num1 = strategy0(score0, score1)
            score0 += take_turn(num1, score0, score1, dice, goal)
            score0 += pigs_on_prime(score0, score1)
        else:
            num2 = strategy1(score1, score0)
            score1 += take_turn(num2, score1, score0, dice, goal)
            score1 += pigs_on_prime(score1, score0)
        who = next_player(who)     
        leader, mes = say(score0, score1, leader)
        if mes != None and mes != "":
            print(mes)
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    num, max_num,cur,max = 1, 1, 0, 0
    while num <= 10:
        cur = make_averaged(roll_dice, total_samples)(num, dice)
        if cur > max:
            max = cur
            max_num = num
        num += 1
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if is_prime(score + oink_points(score, opponent_score)):
        return 0
    elif oink_points(score, opponent_score) >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
