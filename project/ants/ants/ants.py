...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, lower_bound=lower_bound, upper_bound=upper_bound):
        ...
        distance = 0
        while target_place is not None:
            if target_place.bees != [] and target_place.is_hive == False and self.lower_bound <= distance <= self.upper_bound:
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if gamestate.queenant:
            return None
        else:
            queen = super(QueenAnt, cls).construct(gamestate)
            gamestate.queenant = True
            return queen 
        # END Problem 12
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        super().action(gamestate)
        flag = self.place.exit
        ants = []
        while flag:
            if flag.ant:
                ants.append(flag.ant)
            flag = flag.exit 
        for each in ants:
            if not each.is_container:
                each.double()  
            elif each.is_container and each.ant_contained:
                each.ant_contained.double() 
                each.double()
            else:
                each.double()
        # END Problem 12
